...
AIはアルファベータ法で数手先まで読む。評価は、隅やその周辺といったマスの価値、打てる手の多さ(機動力)、石数の差を、終盤ほど石数を重く見るように合成して決める。残りが約10マスになる終盤では、固定の深さを超えて最後まで読み切り、勝敗を確定させて指す。乱数を使わないので、同じ局面からは必ず同じ手を選ぶ。強さは読みの深さで3段階に切り替えられ、AI同士を対戦させて観戦することもできる。
...
| AI同士の対戦を観る | 両者をAIにして対局を観戦する            |
| 棋譜をクリック | その手数の局面に戻って検討する              |
| URLを共有    | 今の対局を載せたリンクをコピーする            |
| 棋譜をコピー | 着手の一覧をテキストでコピーする              |

キーボードでは u=待った、r=進む、h=ヒント、n=新規対局が使え、検討中は左右の矢印キーで棋譜を1手ずつ行き来できる。評価値は黒から見た値で示し、正なら黒有利、負なら白有利、終盤で勝敗が確定すると「勝勢」と表示する。
...
- **終盤は読み切る** — 残りが約10マスになると、指定の深さを超えて空きマス数だけ探索を伸ばし、最後まで読んで勝敗を確定させる。序中盤は固定深さの全幅探索で速さを保つ。
...
- AIは序中盤が固定深さの全幅アルファベータ法で、終盤(残り約10マス)のみ最後まで読み切る。定石は持たず、最強の対戦相手を目指したものではない。
